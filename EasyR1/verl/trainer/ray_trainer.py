...
from ..utils.rollout_trajectory import save_rollout_trajectory_json
...
class RayPPOTrainer:
    ...
    def _maybe_log_rollout_trajectory_json(
        self,
        batch: DataProto,
        reward_metrics: Optional[dict[str, list[Any]]] = None,
        reward_extras: Optional[dict[str, list[Any]]] = None,
    ) -> None:
        if not self.config.trainer.log_rollout_trajectory_json:
            return

        steps = self.config.trainer.rollout_trajectory_json_steps
        if steps is None:
            steps = [1]
        if self.global_step not in steps:
            return

        output_dir = os.path.join(self.config.trainer.save_checkpoint_path, "rollout_trajectories")
        save_rollout_trajectory_json(
            batch=batch,
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            if self.config.algorithm.online_filtering:
                reward_tensor, reward_metrics, _ = ray.get(self.reward_fn.compute_reward.remote(new_batch))
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    rollout_reward_metrics: Optional[dict[str, list[Any]]] = None
                    rollout_reward_extras: Optional[dict[str, list[Any]]] = None
                    if "token_level_scores" not in batch.batch:
                        # get token level scores asynchronously
                        reward_result = ray.get(reward_ref)
                        # 兼容旧返回 (tensor, metrics) 和新返回 (tensor, metrics, extras)
                        if len(reward_result) == 3:
                            reward_tensor, rollout_reward_metrics, rollout_reward_extras = reward_result
                        else:
                            reward_tensor, rollout_reward_metrics = reward_result
                            rollout_reward_extras = {}
                        batch.batch["token_level_scores"] = reward_tensor
                        reward_metrics = {
                            f"reward/{k}": v for k, v in reduce_metrics(rollout_reward_metrics).items()
                        }
                    ...
                    self._maybe_log_rollout_trajectory_json(
                        batch,
